...
class DepthWiseConv_No_ReLu(nn.Cell):
    """
    ...
    def __init__(self, in_planes, out_planes, kernel_size=3, stride=1, padding=0):
        super().__init__()
        self.ds_conv = nn.Conv2d(in_planes, in_planes, kernel_size, group=in_planes,has_bias=True, padding=padding,pad_mode="pad")
        self.ds_bn = nn.BatchNorm2d(in_planes)
        self.pw_conv = nn.Conv2d(in_planes, out_planes, kernel_size=1,has_bias=True)
...
class SSD_Head(nn.Cell):
    def __init__(self, n_classes=81, k_list=[4, 6, 6, 6, 6, 6],
        ...
        self.loc = nn.CellList(self.loc)
        self.conf = nn.CellList(self.conf)
        # self._init_weights()
    ...
    def bbox_view(self, src, loc, conf):
        ...
        for s, l, c in zip(src, loc, conf):
            # print(type(s))
            ret.append((l(s).view(s.shape[0], 4, -1), c(s).view(s.shape[0], self.label_num, -1)))

        locs, confs = list(zip(*ret))
        locs, confs = ops.cat(locs, 2).contiguous(), ops.cat(confs, 2).contiguous()
        return locs, confs
