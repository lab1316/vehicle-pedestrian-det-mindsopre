...
class CocoDetection(VisionDataset):
    ...
    def __getitem__(self, batched_indices):
        ...
        for index in batched_indices:
            ...
            image_transform = transforms.Compose([to_tensor,normalize])
        ...
        # B x C x H x W
        batch_images = ops.stack(imgs)
        batch_images = ms.Tensor(batch_images,dtype=ms.float32)
        # B x #anchors x 4 and 1 respectively
        batch_bboxes = ops.stack(targets_bboxes)
        batch_class_ids = ops.stack(targets_classes)
