...
class Model_evaluator():
    ...
    def complete_evaluate(self, model, optimizer, epoch=0):
        ...
        # with torch.no_grad():
        losses = [0] * 4
        val_set_size =self.valid_loader.set_size

        prediction_annotations = []
        prediction_id = 0

        print(datetime.datetime.now())
        for batch_idx, input_, label ,image_info in self.valid_loader:
            # batch_idx, input_, label ,image_info=next(self.valid_loader)
