...
def map_to_ground_truth(overlaps, gt_bbox, gt_class, params):
   ...
   - matched_gt_class_ids: tensor of size priors - where each value of the tensor indicates the class id that the priors feature map cell should predict
    ...
    gt_to_prior_overlap, gt_to_prior_idxs = overlaps.max(axis=1,return_indices=True)

    # for each prior, what is the object of maximum overlap
    prior_to_gt_overlaps, prior_to_gt_idxs = overlaps.max(axis=0,return_indices=True)
    prior_to_gt_overlap = ms.Tensor(prior_to_gt_overlaps)
    gt_to_prior_idx = ms.Tensor(gt_to_prior_idxs)
    prior_to_gt_idx = ms.Tensor(prior_to_gt_idxs)
    # for priors of max overlap, set a high value to make sure they match
    prior_to_gt_overlap[gt_to_prior_idx] = 1.99
    idx = ops.arange(0, gt_to_prior_idx.shape[0], dtype=ms.int64)
    # if overlaps.is_cuda:
    #     idx = idx.to("cuda:0")
...
class DefaultBoxes(object):
    ...
    def __init__(self, fig_size, feat_size, steps, scales,
        ...
        self.dboxes = ms.Tensor(self.default_boxes)
        ops.clamp(self.dboxes,min=0,max=1)
        # self.dboxes.clamp_(min=0, max=1)
        # For IoU calculation
        self.dboxes_ltrb = ms.Tensor(shape=self.dboxes.shape, dtype=ms.float32, init=One())
        self.dboxes_ltrb = self.dboxes
...
def prepare_gt(input_img, gt_bboxes, gt_classes):
    ...
    """
    gt = [Tensor(np.array(gt_bboxes),dtype=ms.float32), Tensor(np.array(gt_classes),dtype=ms.int32)]
    ...
    for idx, bbox in enumerate(gt[0]):
        ...
        gt[0][idx] = Tensor(np.array(new_bbox).astype(np.float32),dtype=ms.float32)
...
def map_id_to_idx(class_ids):
    ...
    """
    class_idx = ops.zeros(class_ids.shape, dtype=ms.int64)
