...
def wh2corners(ctr, wh):
    # taken from fastai
    return ops.cat([ctr-wh/2, ctr+wh/2], axis=1)
...
def intersect(box_a, box_b):
    ...
    A = box_a.shape[0]
    B = box_b.shape[0]
    max_xy = np.minimum(box_a[:, 2:].unsqueeze(1).broadcast_to((A, B, 2)).asnumpy(),
                       box_b[:, 2:].unsqueeze(0).broadcast_to((A, B, 2)).asnumpy())
    min_xy = np.maximum(box_a[:, :2].unsqueeze(1).broadcast_to((A, B, 2)).asnumpy(),
                       box_b[:, :2].unsqueeze(0).broadcast_to((A, B, 2)).asnumpy())
    max_xy = ms.Tensor(max_xy)
    min_xy = ms.Tensor(min_xy)
...
def jaccard(box_a, box_b):
    ...
    b = (box_a[:, 2]-box_a[:, 0]) *(box_a[:, 3]-box_a[:, 1]).unsqueeze(1)
